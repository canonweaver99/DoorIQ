-- Update free plan from 10 to 5 credits per month
-- Migration: 048_update_free_plan_to_5_credits

-- Update the default session limit for all existing free users
UPDATE user_session_limits
SET sessions_limit = 5
WHERE sessions_limit = 10;

-- Update the check_user_session_limit function to use 5 as default
...
BEGIN
  ...
  -- Get or create limit record with 5 credits default
...
-- Update the increment_user_session_count function to use 5 as default
...
COMMENT ON FUNCTION check_user_session_limit(UUID) IS 'Updated to 5 credits per month for free users - Migration 048';
COMMENT ON FUNCTION increment_user_session_count(UUID) IS 'Updated to 5 credits per month for free users - Migration 048';
