-- Add learning_page feature flag
INSERT INTO feature_flags (feature_key, feature_name, description, requires_subscription, enabled_for_trial, enabled_for_free)
VALUES 
  ('learning_page', 'Custom Sales Playbook (Learning Page)', 'Access to the Learning page with custom sales playbook features', TRUE, TRUE, FALSE)
ON CONFLICT (feature_key) DO NOTHING;

-- Update user_has_feature_access function to check organization plan_tier for learning_page
...
BEGIN
  ...
  IF p_feature_key = 'learning_page' THEN
    ...
    IF v_organization_id IS NOT NULL THEN
      ...
      -- Learning page only available for team and enterprise plans, not starter
      IF v_plan_tier = 'starter' THEN
        RETURN FALSE;
      END IF;
      
      -- For team and enterprise, check subscription status
      IF v_plan_tier IN ('team', 'enterprise') THEN
