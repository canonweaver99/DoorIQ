...
export const IPhoneMockup: React.FC<IPhoneMockupProps> = ({
  ...
  // Enhanced metallic gradient for realistic iPhone look
  const frameGradient = `linear-gradient(135deg, 
    ${shade(colorHex, 15)} 0%, 
    ${shade(colorHex, 8)} 20%, 
    ${colorHex} 40%, 
    ${shade(colorHex, -8)} 60%, 
    ${shade(colorHex, -12)} 80%, 
    ${shade(colorHex, -18)} 100%
  )`;
  ...
  const innerShadowCss = innerShadow
    ? 'inset 0 0 0 1px rgba(255,255,255,0.08), inset 0 1px 2px rgba(255,255,255,0.1), inset 0 10px 20px rgba(0,0,0,0.35), inset 0 -8px 16px rgba(0,0,0,0.28)'
    : 'none';
