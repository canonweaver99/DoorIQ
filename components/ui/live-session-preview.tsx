...
export function LiveSessionPreview() {
  ...
  return (
    <div className="relative w-full h-full flex flex-col bg-black/40 backdrop-blur-sm rounded-2xl border border-purple-500/20 overflow-hidden" style={{ minHeight: 'calc(100vh - 120px)', maxHeight: 'calc(100vh - 120px)', transform: 'scale(1.05)', transformOrigin: 'center' }}>
      {/* Header */}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top Section: 50/50 Split - Sales Rep Left, Agent Right */}
        <div className="grid grid-cols-2 gap-0 border-b border-purple-500/20" style={{ minHeight: '300px' }}>
          {/* Left: Sales Rep */}
          <div className="flex flex-col items-center justify-center p-6 border-r border-purple-500/20 bg-gradient-to-br from-green-950/20 to-transparent relative">
            <div className="relative mb-6">
              ...
              <div className={`relative w-40 h-40 rounded-full transition-all duration-500 ${
                ...
                <div className="relative w-full h-full rounded-full overflow-hidden bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-green-500/50">
                  {/* Webcam Feed Placeholder - Grid Pattern */}
          ...
          {/* Right: Agent (Austin) */}
          <div className="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-purple-950/20 to-transparent">
            <div className="relative mb-6">
              {/* Breathing animated rings */}
              {[0, 1, 2].map((i) => (
                <motion.div
          ...
          >
            {transcript.slice(0, visibleMessages).map((entry, index) => {
              ...
              return (
                <motion.div
                key={index}
                  initial={{ opacity: 0, x: entry.speaker === 'austin' ? -10 : 10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.3 }}
                  className={`flex ${entry.speaker === 'austin' ? 'justify-start' : 'justify-end'}`}
                >
                  <div className={`max-w-[85%] p-2.5 rounded-lg ${
                    entry.speaker === 'austin'
                      ? 'bg-purple-500/20 border border-purple-500/30' 
                      : 'bg-green-500/20 border border-green-500/30'
                  }`}>
                  <div className="flex items-center gap-2 mb-1">
                      <span className={`text-xs font-semibold ${
                        entry.speaker === 'austin' ? 'text-purple-400' : 'text-green-400'
                    }`}>
                        {entry.speaker === 'austin' ? 'Austin' : 'Sales Rep'}
                    </span>
                    <span className="text-[10px] text-slate-500">{entry.time}</span>
                  </div>
