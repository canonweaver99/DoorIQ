...
function MetricBar({ label, score, color, relatedLines = [], feedback, onLineClick }: MetricProps & { onLineClick?: (lineNumber: number) => void }) {
  ...
  const colorClasses = {
    blue: { bar: 'bg-blue-500', bg: 'bg-blue-900/30', text: 'text-blue-100', border: 'border-blue-700/50' },
    green: { bar: 'bg-green-500', bg: 'bg-green-900/30', text: 'text-green-100', border: 'border-green-700/50' },
    yellow: { bar: 'bg-yellow-500', bg: 'bg-yellow-900/30', text: 'text-yellow-100', border: 'border-yellow-700/50' },
    purple: { bar: 'bg-purple-500', bg: 'bg-purple-900/30', text: 'text-purple-100', border: 'border-purple-700/50' },
  }
    ...
    <div className="mb-6">
      ...
      <AnimatePresence>
        {isExpanded && hasRelatedLines && (
          ...
          >
            <div className={`ml-4 mt-2 p-4 border-l-4 ${colors.border} bg-slate-700 rounded-r-lg shadow-sm`}>
              ...
              <div className="space-y-3">
                {relatedLines.map((line, idx) => (
                  <motion.div
                    ...
                    className="flex items-start space-x-3 p-3 bg-slate-800 rounded-lg hover:bg-slate-600 transition-colors cursor-pointer"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                          line.impact === 'positive' ? 'bg-green-900/50 text-green-300' :
                          line.impact === 'negative' ? 'bg-red-900/50 text-red-300' :
                      ...
                      <div className="text-sm text-slate-200 italic mb-2 line-clamp-2">
...
export default function PerformanceMetrics({ 
  ...
  return (
    <div className={`bg-slate-800 rounded-xl shadow-xl p-8 border border-slate-700 ${className}`}>
      <h2 className="text-2xl font-semibold text-slate-100 mb-6">Performance Breakdown</h2>
